    ...
    formatCityList (cities) {
      ...
      for (let i = 0; i < cities.length; i++) {
        ...
        if (toCom(firstLetter)) {
          // 新增加index
          cityList.push({ index: firstLetter, list: [{ name: cities[i].name, id: cities[i].cityId, pinyin: cities[i].pinyin }] })
        } else {
          // 累加到已有index中
          for (let j = 0; j < cityList.length; j++) {
            if (cityList[j].index === firstLetter) {
              cityList[j].list.push({ name: cities[i].name, id: cities[i].cityId, pinyin: cities[i].pinyin })
            }
          }
        }
      }
